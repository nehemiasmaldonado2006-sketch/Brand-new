...
import { CMA_TEMPLATES } from "@/lib/marketing/templates";
...
export default async function CmaLibraryPage() {
  const session = await auth();
  const userId = session?.user?.id;
  const hidden = userId ? await getHiddenTemplateKeys(userId) : new Set<string>();
  const templates = CMA_TEMPLATES.filter((t) => !hidden.has(t.key));
      ...
      <main className="flex flex-col gap-[22px] px-11 pt-[26px] pb-[60px]">
        ...
        <div className="flex flex-wrap items-end justify-between gap-5">
          <div>
            ...
            <div className="font-serif text-[34px] leading-none">CMA templates</div>
            <div className="mt-1.5 text-[12.5px] text-muted">
              CMA layouts auto-fill from the subject property and comps entered in the CMA
              Builder.
            </div>
          </div>
          <Link
            href="/marketing/studio/new?kind=cma"
            ...
            <article key={t.key} className="flex flex-col border border-black/[0.12] bg-panel">
              ...
              <div className="flex flex-1 flex-col gap-2.5 px-3.5 py-3">
                ...
                <div className="mt-auto flex gap-2">
                  <Link
                    href={`/marketing/studio/new?kind=cma&template=${t.key}`}
                  ...
                  {userId && (
                    <form action={hideTemplate}>
                      ...
                      <input type="hidden" name="returnTo" value="/marketing/cma" />
                      <button
                        type="submit"
